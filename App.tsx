...
import { SkillMode, UserStats, AIModelType, SUPPORTED_MODELS } from './types';
...
function App() {
   ...
   useEffect(() => {
      ...
      const localModel = localStorage.getItem('lingua_selected_model') as AIModelType;

      setApiKey(localKey);
      if (localModel && SUPPORTED_MODELS.some(m => m.id === localModel)) {
         setSelectedModel(localModel);
      }

      // 3. Check if Key missing -> Open Modal
      // We assume if env var is set, localKey might be empty but getApiKey() returns it.
      // But getApiKey() reads env too.
      // The requirement says "Lưu vào localStorage... hiển thị Modal bắt buộc nhập". 
      // This implies we rely on user input if not present.
      // If getApiKey() returns empty string, we MUST open modal.
      if (!getApiKey() && !localKey) {
   ...
   const currentModelName = SUPPORTED_MODELS.find(m => m.id === selectedModel)?.name || 'Generic AI';
   ...
   if (!currentUser) {
      return (
         <>
            ...
            {/* Even in Login, we might want to config key? Maybe not. Requirement says "Header". Login usually has no header. */}
         </>
      );
   }
               ...
               <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                  ...
                  <div className="flex items-center gap-4">
                     ...
                     <button
                        ...
                        className={`px-5 py-2.5 rounded-full border shadow-sm flex items-center gap-3 transition-all hover:-translate-y-0.5 hover:shadow-md ${!apiKey
                              ? 'bg-red-50 border-red-200 text-red-600 animate-pulse' // Flashing red if no key
                              : 'bg-white border-gray-200 text-gray-700 hover:border-blue-300'
                           }`}
                     >
                        <div className="relative flex h-3 w-3">
                           <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${!apiKey ? 'bg-red-400' : 'bg-green-400'}`}></span>
                           <span className={`relative inline-flex rounded-full h-3 w-3 ${!apiKey ? 'bg-red-500' : 'bg-green-500'}`}></span>
                        </div>
                        <span className="text-sm font-bold">
                           {!apiKey ? "Lấy API key để sử dụng app" : "Settings"}
                        </span>
                     ...
                     {/* Model Badge */}
                     {apiKey && (
                        <div className="hidden md:flex px-4 py-2.5 bg-blue-50 text-blue-700 rounded-full border border-blue-100 font-bold text-xs uppercase tracking-wide items-center gap-2">
